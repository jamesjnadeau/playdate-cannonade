...
-- see its header for why GameSceneMain/GameSceneTraining don't use the real
-- source/scenes/GameScene.lua), and finally the *real* scene files under
-- test -- in the same order source/main.lua imports them, since a few
-- top-level scene statements (e.g. GameSceneMain.inputHandler =
-- GameScene.buildSharedInputHandler(...)) run at load time and need GameScene
-- to already exist.
--
